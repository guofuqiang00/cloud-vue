(function(win) {
  ...
  Client.prototype.createConnect = function(max, delay) {
    ...
    function connect() {
      if (IsPC() && (IEVersion() < 10 && IEVersion() > 0)) {
        // WEB_SOCKET_SWF_LOCATION = window.jsHost + "/js/pc/WebSocketMain.swf";
        console.log('当前浏览器不支持弹幕');
        return;
      }
      var ws = new WebSocket('wss://dm.smzb.net:3103');
      ...
      // }
      ws.onmessage = function(evt) {
        var data = _base64ToArrayBuffer(evt.data);
        var dataView = new DataView(data, 0);
        var packetLen = dataView.getInt32(packetOffset);
        var headerLen = dataView.getInt16(headerOffset);
        var ver = dataView.getInt16(verOffset);
      ...
      function decode(base64) {
        ...
        decode = window.btoa(unescape(encodeURIComponent(decode)));
        return decodeURIComponent(escape(window.atob(decode)));
        // 编码转字符串
        var str = (decode);
        return str;
      }
      ...
      function auth() {
        var token = '{"mid":' + userId + ', "room_id":"live://' + roomId +
          '", "platform":"web", "accepts":[1000,1001,1002]}'
        var headerBuf = new ArrayBuffer(rawHeaderLen);
        var headerView = new DataView(headerBuf, 0);
...
function IsPC() {
  ...
  if (window.screen.width < 500) { //避免那些可以改UA的浏览器修改了UA，用屏幕开发宽度再判断一下
    flag = false;
  }
  return flag;
}
